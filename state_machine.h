...
struct State {
    ...
    handleCharacter (char next, std::string &accumulator, StateContext &context,
...
struct StateContext {
    ...
    StateContext () { state = std::make_unique<InitState> (); }

    void setState (State *new_state) { state.reset (new_state); }

    std::shared_ptr<Node> handleCharacter (char next, std::string &accumulator,
